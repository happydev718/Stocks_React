...
const rankdataurl = configURL.teamoftheseason;
...
class TeamoftheSeasonlist extends Component {
	constructor(props) {
		super(props);
		this.state = {
			rankdata: null,
	...
	async getRankingDate() {
		let query_rankingdate = await db.collection("rankingsdate").doc("teamoftheseason").get();
										...
										<tbody>
											{!this.state.isloading ? playerdatalist.map((obj, key) => {
												...
												if (!splitdata[0]) return <></>;
													...
													return (
														<TeamOfTheSeason data={splitdata} />
													);
											}) : <Spinner type="table" />}
										</tbody>
									</table>
...
export default connect(mapStateToProps)(TeamoftheSeasonlist);
