...
import {Nav, Tab, CloseButton, Row, Button} from 'react-bootstrap';
...
class HistoryOrder extends Component {
	...
	async loadMyOpenOrders(id) {
		try {
			let snapshot_myOpenOrders = await db.collection("OpenOrder").doc(id).collection(this.props.account).get();
			...
			console.log("HistoryOrder.js loadMyOpenOrders error = : ", error);
		}
	}
						...
						this.props.platform === "mobile" ? <tbody>
								...
								<div key={index} className="pt-3">
									<Row className="m-0 p-1">
										<Row className="m-0 p-0 w-60">
											...
											<div className="fs-4 my-auto">{moment.unix(order.timestamp).format('h:mm:ss a')} <span className="text-muted fs-7 my-auto">{moment.unix(order.timestamp).format('M/D/Y')}</span></div>
										</Row>
									...
									<Row className="m-0 p-1">
										<Row className="m-0 p-0 w-60">
											...
											<div className="fs-4 my-auto"  style={{marginBottom: '0px !important'}}>£{isNaN(order.price) ? '0.00' : (parseFloat(order.price)).toFixed(2).toString()} <span className="text-muted fs-7 my-auto">${isNaN(parseFloat(order.price) * parseFloat(this.state.ethPriceData)) ? "0.00" : (parseFloat(order.price) * parseFloat(this.state.ethPriceData)).toFixed(2)}</span></div>
										</Row>
										<Row className="fw-medium text-capitalize m-0 p-0 w-40">
											<div className="text-muted fs-4 my-auto">{"Amount:"}</div>
											<div className="fs-4 my-auto">{order.transaction_type === 'buy' ? "+" : "-"}{ isNaN(order.amount) ? "0" : parseFloat(order.amount).toFixed(2)}</div>
										</Row>
									</Row>
							...
							{this.state.myFilledOrders.map((order, index) => (
								<tr key={index}>
									...
									<td>
										£{isNaN(parseFloat(order.price)) ? "0.00" : (parseFloat(order.price)).toFixed(2)}
									</td>
									<td>
										<span className="text-muted">
											{order.transaction_type === "Buy" ? "+" : "-"}{isNaN(parseFloat(order.amount)) ? "0.00" : (parseFloat(order.amount)).toFixed(2)}
										</span>
									</td>
								</tr>
							))}
							...
							{this.state.myOpenOrders.map((order, index) => (
								<div key={index} className="pt-3">
									...
									<Row className="m-0 p-1">
										<Row className="m-0 p-0 w-60">
											<div className="text-muted fs-4 my-auto">{"Price:"}</div>
											<div className="fs-4 my-auto">£{(parseFloat(ether(order._priceInWei))).toFixed(2).toString()} <span className="text-muted fs-7 my-auto">${(parseFloat(ether(order._priceInWei)) * parseFloat(this.state.ethPriceData)).toFixed(2)}</span></div>
										</Row>
										<Row className="fw-medium text-capitalize m-0 p-0 w-40">
											<div className="text-muted fs-4 my-auto">{"Amount:"}</div>
	...
	render() {
		return (
			<div className="history-order card card--overflow">
				<Tab.Container id="market-history" defaultActiveKey="open-orders">
					<div className="card-header">
						<Nav variant="tabs">
							...
							<Nav.Item>
								<Nav.Link eventKey="open-history">Open History</Nav.Link>
							</Nav.Item>
						</Nav>
					</div>
					...
					<Tab.Content className="flex-grow-scroll">
						...
						<Tab.Pane eventKey="open-history">
							{(this.state.myFilledOrders !== undefined && this.state.myFilledOrders !== null && !this.props.orderCancelling) ?
